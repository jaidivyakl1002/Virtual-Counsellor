...
class NaukriJobScraper:
    ...
    async def scrape_job_urls(self, search_term="python", location="India", results_wanted=50):
        ...
        jobs.to_csv("jobs_urls.csv", quoting=csv.QUOTE_NONNUMERIC, escapechar="\\", index=False)
        ...
        try:
            async with AsyncWebCrawler(config=self.browser_config) as crawler:
                ...
                crawler_config = CrawlerRunConfig(
                    ...
                    css_selector=".styles_job-header-container___0wLZ",
                    ...
                    page_timeout=30000,
                    wait_for="css:.styles_job-header-container___0wLZ"
                )
        ...
        try:
            async with AsyncWebCrawler(config=self.browser_config) as crawler:
                ...
                crawler_config = CrawlerRunConfig(
                    cache_mode=CacheMode.BYPASS,
                    # Target the main job description container
                    css_selector=".styles_job-desc-container__txpYf",
                    ...
                    page_timeout=30000,
                    wait_for="css:.styles_job-desc-container__txpYf"
                )
    ...
    async def scrape_job_batch(self, urls: List[str], batch_size: int = 5, delay: float = 2.0) -> List[dict]:
        ...
        for i in range(0, total_urls, batch_size):
            ...
            for idx, (url, job_card_task, jd_card_task) in enumerate(tasks):
                try:
                    job_card_result, jd_result = await asyncio.gather(job_card_task, jd_card_task)
                    
                    # Combine results
                    combined_result = {
                        'url': url,
                        'success': True,
                        'job_header_markdown': job_card_result.markdown if hasattr(job_card_result, 'markdown') else '',
                        'job_description_markdown': jd_result.markdown if hasattr(jd_result, 'markdown') else '',
                        'error': None
                    }
                    ...
                    self.save_individual_markdown(combined_result, filename, markdown_dir)
            ...
            print(f"Batch {batch_num} completed: {successful}/{len(batch_urls)} successful")
    ...
    def save_results(self, results: List[dict], output_file: str = "job_descriptions.csv"):
...
async def main():
    ...
    try:
        ...
        job_urls = await scraper.scrape_job_urls(
            ...
            location="India",
            results_wanted=3  # Start small for testing
        )
