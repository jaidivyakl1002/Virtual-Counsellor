...
class TestScoreInterpretationOutput(BaseModel):
    ...
    executive_summary: str = Field(description="Brief overview of student's overall profile")
    aptitude_analysis: Dict[str, Any] = Field(description="DBDA aptitude test interpretation")
    interest_analysis: Dict[str, Any] = Field(description="CII interest inventory interpretation") 
    aptitude_interest_alignment: Dict[str, Any] = Field(description="How aptitudes align with interests")
    psychological_insights: Dict[str, Any] = Field(description="Age-appropriate psychological insights")
    developmental_considerations: List[str] = Field(description="Adolescent development factors")
    confidence_indicators: Dict[str, Any] = Field(description="Reliability of assessment results")
    key_recommendations: List[str] = Field(description="Primary recommendations for next steps")
...
class TestScoreInterpreterAgent(BaseAgent):
    ...
    def _define_optional_inputs(self) -> List[str]:
        ...
        return [
            "student_name", "age", "current_grade", "academic_performance",
            "extracurricular_activities", "family_background", "test_taking_conditions"
        ]
    ...
    def _initialize_agent(self):
        ...
        self.output_parser = JsonOutputParser(pydantic_object=TestScoreInterpretationOutput)
        
        # Create the main interpretation prompt
        self.interpretation_prompt = PromptTemplate(
            input_variables=[
                "student_info", "dbda_scores", "cii_scores", 
                "cii_domains", "dbda_domains", "optional_context"
            ],
...
Remember: This student is at a crucial stage of identity and career development. Your interpretation should inspire confidence while providing realistic guidance for their educational and career journey."""
    ...
    def _process_core_logic(self, validated_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        cii_formatted = self._format_sten_scores(cii_results, "Career Interest Inventory (CII)")
        ...
        try:
            ...
            result["interpretation_metadata"] = {
                ...
                "assessment_types": ["DBDA Aptitude Test", "Career Interest Inventory (CII)"],
                "interpretation_date": validated_input.get("session_metadata", {}).get("timestamp", "unknown"),
                ...
                "reliability_factors": self._assess_test_reliability(dbda_scores, cii_results, optional_data)
            }
            
            # Add score summaries for quick reference
            result["score_summaries"] = {
                "dbda_top_aptitudes": self._identify_top_scores(dbda_scores, self.dbda_domains),
                "cii_top_interests": self._identify_top_scores(cii_results, self.cii_domains),
                "aptitude_interest_patterns": self._identify_aptitude_interest_patterns(dbda_scores, cii_results)
            }
    ...
    def _prepare_student_info(self, optional_data: Dict[str, Any]) -> str:
        ...
        if optional_data.get("academic_performance"):
            info_parts.append(f"Academic Performance: {optional_data['academic_performance']}")
    ...
    def _format_domains(self, domains: Dict[str, str]) -> str:
        ...
        for domain, description in domains.items():
            formatted_lines.append(f"- {domain.replace('_', ' ').title()}: {description}")
    ...
    def _prepare_optional_context(self, optional_data: Dict[str, Any]) -> str:
        ...
        if optional_data.get("test_taking_conditions"):
            context_parts.append(f"Test Conditions: {optional_data['test_taking_conditions']}")
    ...
    def _identify_top_scores(self, scores: Dict[str, Any], domain_descriptions: Dict[str, str]) -> List[Dict[str, Any]]:
        ...
        for domain, score in sorted_scores:
            top_areas.append({
                ...
                "score": score,
                "description": domain_descriptions.get(domain, "Description not available"),
                "level": self._categorize_sten_score(score)
            })
