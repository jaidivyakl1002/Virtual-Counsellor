...
from agentic_layer.school_students.agents.sub_agents.practical_guidance_sub_agent import PracticalGuidanceSubAgent
from agentic_layer.school_students.agents.sub_agents.career_readiness_sub_agent import CareerReadinessSubAgent
...
class CareerPathway(BaseModel):
    ...
    suitability_score: float = Field(description="Suitability based on student profile (0.0-1.0)")
    pathway_description: str = Field(description="What this career involves day-to-day")
    educational_requirements: List[str] = Field(description="Required education and qualifications")
    entry_timeline: str = Field(description="Time from 12th grade to entry-level position")
    ...
    salary_outlook: Dict[str, str] = Field(description="Salary ranges at different career stages")
    job_market_outlook: str = Field(description="Future demand and opportunities")
    challenges_considerations: List[str] = Field(description="Potential challenges to consider")
    success_factors: List[str] = Field(description="What leads to success in this career")
...
class CareerExplorationInsights(BaseModel):
    ...
    aptitude_career_mapping: Dict[str, List[str]] = Field(description="How aptitudes map to career clusters")
    interest_career_alignment: Dict[str, List[str]] = Field(description="How interests align with career options")
    stream_career_pathways: Dict[str, List[str]] = Field(description="Career pathways from recommended streams")
    emerging_opportunities: List[str] = Field(description="New and emerging career opportunities")
    skill_development_priorities: List[str] = Field(description="Skills to develop for target careers")

class CareerPathwayExplorerOutput(BaseModel):
    """Structured output for career pathway exploration"""
    executive_summary: str = Field(description="Overview of career exploration findings")
    recommended_career_pathways: List[CareerPathway] = Field(description="Top career pathway recommendations")
    career_exploration_insights: CareerExplorationInsights = Field(description="Analytical insights")
    career_readiness_assessment: Dict[str, Any] = Field(description="Student's readiness for different career paths")
    exploration_activities: List[str] = Field(description="Suggested career exploration activities")
    mentorship_recommendations: List[str] = Field(description="Types of mentors to seek")
    timeline_planning: Dict[str, Any] = Field(description="Career planning timeline considerations")
    decision_support_framework: Dict[str, Any] = Field(description="Framework for career decision making")
    next_steps: List[str] = Field(description="Immediate next steps for career exploration")
...
class CareerPathwayExplorerAgent(BaseAgent):
    ...
    def __init__(self, llm_model=None, config: Dict[str, Any] = None):
        ...
        self.stream_career_mapping = {
            "Science (PCM)": {
                "primary_careers": [
                    "Engineering (All branches)", "Architecture", "Computer Science", "Data Science",
                    "Aerospace Technology", "Robotics", "AI/ML Specialist", "Research Scientist",
                    "Patent Attorney", "Technical Consultant", "Product Manager (Tech)"
                ],
                "emerging_careers": [
                    "Cybersecurity Specialist", "Blockchain Developer", "IoT Engineer", 
                    "Space Technology", "Clean Energy Engineer", "Quantum Computing"
                ],
                "entry_pathways": ["JEE Main/Advanced", "State Engineering Entrance", "BITSAT", "Private University Exams"]
            },
            "Science (PCB)": {
                "primary_careers": [
                    "Doctor (MBBS)", "Dentist (BDS)", "Veterinarian", "Pharmacist", "Physiotherapist",
                    "Biotechnologist", "Microbiologist", "Genetic Counselor", "Medical Research",
                    "Clinical Psychology", "Nutrition Specialist", "Public Health"
                ],
                "emerging_careers": [
                    "Bioinformatics Specialist", "Stem Cell Researcher", "Precision Medicine", 
                    "Digital Health", "Telemedicine", "Biomedical Engineering"
                ],
                "entry_pathways": ["NEET-UG", "NEET-PG", "State Medical Entrance", "AIIMS", "JIPMER"]
            },
            "Science (PCMB)": {
                "primary_careers": [
                    "Medical Engineering", "Biomedical Engineering", "Biotechnology", "Bioengineering",
                    "Medical Research", "Healthcare Technology", "Pharmaceutical Engineering",
                    "Clinical Data Analysis", "Medical Device Development"
                ],
                "emerging_careers": [
                    "Healthcare AI", "Digital Therapeutics", "Personalized Medicine", 
                    "Medical Robotics", "Regenerative Medicine"
                ],
                "entry_pathways": ["JEE + NEET options", "Dual degree programs", "Integrated courses"]
            },
            "Commerce (Math)": {
                "primary_careers": [
                    "Chartered Accountant", "Investment Banker", "Financial Analyst", "Actuary",
                    "Business Analyst", "Management Consultant", "Economics Researcher",
                    "Data Analyst", "Risk Manager", "Corporate Finance", "Entrepreneur"
                ],
                "emerging_careers": [
                    "Fintech Specialist", "Cryptocurrency Analyst", "ESG Analyst", 
                    "Algorithmic Trading", "Digital Banking", "Startup Ecosystem"
                ],
                "entry_pathways": ["CA Foundation", "CMA", "Company Secretary", "BBA/BMS", "Economics Honors"]
            },
            "Commerce (No Math)": {
                "primary_careers": [
                    "Human Resources", "Marketing Manager", "Sales Manager", "Business Development",
                    "Retail Management", "Event Management", "Public Relations", "Digital Marketing",
                    "Content Marketing", "Social Media Strategy", "Brand Management"
                ],
                "emerging_careers": [
                    "Influencer Marketing", "Customer Experience Design", "Growth Hacking",
                    "Social Commerce", "Creator Economy", "Sustainable Business"
                ],
                "entry_pathways": ["BBA/BMS", "Mass Communication", "Hotel Management", "Fashion Management"]
            },
            "Arts/Humanities": {
                "primary_careers": [
                    "Civil Services (IAS/IPS)", "Lawyer", "Journalist", "Teacher/Professor", 
                    "Social Worker", "Counselor", "Historian", "Archaeologist", "Translator",
                    "Foreign Service", "NGO Leadership", "Policy Researcher"
                ],
                "emerging_careers": [
                    "Policy Analyst", "Social Impact Consultant", "Digital Content Creator",
                    "UX Writer", "Cultural Heritage Manager", "Sustainability Consultant"
                ],
                "entry_pathways": ["UPSC", "CLAT", "Mass Communication", "BA Liberal Arts", "Social Work"]
            }
        }
        
        # Aptitude-career cluster mapping
        self.aptitude_career_clusters = {
            "computational": ["Data Science", "Software Engineering", "Financial Analysis", "Research"],
            "experimental": ["Scientific Research", "R&D", "Laboratory Medicine", "Clinical Research"],
            "technical": ["Engineering", "Technology Development", "Technical Consulting", "Product Development"],
            "medical": ["Healthcare", "Medical Research", "Biotechnology", "Public Health"],
            "administrative": ["Management", "Civil Services", "Operations", "Project Management"],
            "humanitarian": ["Social Work", "NGO", "Teaching", "Counseling", "Public Service"],
            "educational": ["Teaching", "Training", "Educational Technology", "Academic Research"],
            "creative": ["Design", "Media", "Arts", "Content Creation", "Marketing"],
            "nature": ["Environmental Science", "Agriculture", "Forestry", "Marine Biology"],
            "clerical": ["Administration", "Documentation", "Compliance", "Operations Support"]
        }
        
        # Interest-career alignment mapping  
        self.interest_career_alignment = {
            "administrative": ["Management", "Civil Services", "Operations", "HR"],
            "entertainment": ["Media", "Film", "Television", "Event Management", "Sports"],
            ...
            "computational": ["IT", "Data Science", "Analytics", "Software Development"],
    ...
    def _define_optional_inputs(self) -> List[str]:
        ...
        return [
            "current_grade", "academic_performance", "extracurricular_activities",
            "career_aspirations", "family_background", "geographical_preferences",
            "financial_considerations", "previous_agent_outputs"
        ]
    ...
    def _initialize_agent(self):
        ...
        self.output_parser = JsonOutputParser(pydantic_object=CareerPathwayExplorerOutput)
        
        # Create the main career exploration prompt
        self.exploration_prompt = PromptTemplate(
            input_variables=[
                "student_profile", "assessment_analysis", "stream_recommendations", 
                "aptitude_insights", "interest_patterns", "career_context", "exploration_framework"
            ],
...
Remember: This exploration should inspire the student while providing practical, actionable guidance for their career journey."""
    ...
    def _process_core_logic(self, validated_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        assessment_analysis = self._extract_assessment_analysis(test_interpreter_result, dbda_scores, cii_results)
        stream_recommendations = self._extract_stream_recommendations(stream_advisor_result)
        ...
        try:
            ...
            result["exploration_metadata"] = {
                ...
                "based_on_assessments": ["DBDA Aptitude Test", "Career Interest Inventory (CII)"],
            ...
            result["computational_analysis"] = {
                "aptitude_career_matching": self._calculate_aptitude_career_scores(dbda_scores),
                "interest_career_alignment": self._calculate_interest_career_scores(cii_results),
                "stream_pathway_analysis": self._analyze_stream_pathways(stream_recommendations, dbda_scores, cii_results),
                "career_readiness_scores": self._calculate_career_readiness(dbda_scores, cii_results, optional_data),
                "top_career_recommendations": self._get_top_career_matches(dbda_scores, cii_results, stream_recommendations)
            }
            ...
            result["practical_guidance"] = self.practical_guidance_agent.generate_guidance(
                student_profile=self._prepare_student_profile(optional_data),
                career_recommendations=result["recommended_career_pathways"],
                assessment_data=assessment_data,
                context=student_data["constraints"]
            )
            self._add_processing_note("Dynamic practical guidance generated successfully")
            ...
            self._add_processing_note("Career pathway exploration completed successfully")
            return result
    ...
    def _prepare_student_profile(self, optional_data: Dict[str, Any]) -> str:
        ...
        if optional_data.get("academic_performance"):
            profile_parts.append(f"Academic Performance: {optional_data['academic_performance']}")
        
        # Extracurricular activities
        if optional_data.get("extracurricular_activities"):
            activities = optional_data["extracurricular_activities"]
            if isinstance(activities, list):
                profile_parts.append(f"Extracurricular Activities: {', '.join(activities)}")
            else:
                profile_parts.append(f"Extracurricular Activities: {activities}")
        
        # Career aspirations
        if optional_data.get("career_aspirations"):
            profile_parts.append(f"Current Career Interests: {optional_data['career_aspirations']}")
        
        # Family background context
        if optional_data.get("family_background"):
            profile_parts.append(f"Family Context: {optional_data['family_background']}")
        
        return "\n".join(profile_parts) if profile_parts else "Basic student profile available for career exploration"
    
    def _extract_assessment_analysis(self, test_interpreter_result, dbda_scores: Dict[str, Any], cii_results: Dict[str, Any]) -> str:
        """Extract key insights from test interpretation for career exploration"""
        if not test_interpreter_result or test_interpreter_result.status != ProcessingStatus.COMPLETED:
            # Provide basic analysis if no interpreter results available
            analysis_parts = ["Assessment analysis based on raw scores:"]
            
            # Analyze DBDA scores
            top_aptitudes = sorted(dbda_scores.items(), key=lambda x: x[1], reverse=True)[:3]
            analysis_parts.append("Top Aptitude Areas:")
            for aptitude, score in top_aptitudes:
                analysis_parts.append(f"- {aptitude.replace('_', ' ').title()}: {score}/10")
            
            # Analyze CII results
            top_interests = sorted(cii_results.items(), key=lambda x: x[1], reverse=True)[:3]
            analysis_parts.append("Top Interest Areas:")
            for interest, score in top_interests:
                analysis_parts.append(f"- {interest.replace('_', ' ').title()}: {score}/10")
        ...
        if output_data.get("executive_summary"):
            analysis_parts.append(f"Assessment Overview: {output_data['executive_summary']}")
        ...
        return "\n".join(analysis_parts) if analysis_parts else "Basic assessment analysis available"
    ...
    def _extract_stream_recommendations(self, stream_advisor_result) -> str:
        ...
        if not stream_advisor_result or stream_advisor_result.status != ProcessingStatus.COMPLETED:
        ...
        if output_data.get("executive_summary"):
            stream_parts.append(f"Stream Advisor Summary: {output_data['executive_summary']}")
            ...
            if isinstance(streams, list):
                for i, stream in enumerate(streams[:3], 1):  # Top 3 streams
                    if isinstance(stream, dict):
                        ...
                        stream_parts.append(f"{i}. {stream_name} (Suitability: {suitability})")
                        
                        # Add supporting reasons if available
                        if stream.get("primary_strengths_supporting"):
                            stream_parts.append(f"   Supporting Strengths: {', '.join(stream['primary_strengths_supporting'][:2])}")
        
        # Computational analysis if available
        if output_data.get("computational_analysis", {}).get("top_3_recommended_streams"):
            top_streams = output_data["computational_analysis"]["top_3_recommended_streams"]
            ...
            for stream_info in top_streams:
                if isinstance(stream_info, dict):
                    ...
                    stream_parts.append(f"- {stream_name}: {score:.2f} suitability score")
        
        return "\n".join(stream_parts) if stream_parts else "Stream recommendations under analysis"
    ...
    def _analyze_aptitude_insights(self, dbda_scores: Dict[str, Any]) -> str:
        ...
        for aptitude, score in top_aptitudes:
            ...
            if aptitude in self.aptitude_career_clusters:
                career_clusters = self.aptitude_career_clusters[aptitude][:3]  # Top 3 clusters
                insights_parts.append(f"  Relevant Career Clusters: {', '.join(career_clusters)}")
            
            # Add interpretation
            if score >= 8:
                insights_parts.append(f"  Strong aptitude - excellent foundation for related careers")
            elif score >= 6:
                insights_parts.append(f"  Good aptitude - suitable for related career paths")
            else:
                insights_parts.append(f"  Developing aptitude - could be strengthened with focused effort")
    ...
    def _analyze_interest_patterns(self, cii_results: Dict[str, Any]) -> str:
        ...
        high_interests = [(interest, score) for interest, score in cii_results.items() if score >= 6]
        high_interests.sort(key=lambda x: x[1], reverse=True)
        
        if not high_interests:
            # If no high interests, show top 3 relative interests
            high_interests = sorted(cii_results.items(), key=lambda x: x[1], reverse=True)[:3]
        ...
        for interest, score in high_interests[:4]:  # Top 4 interests
            ...
            if interest in self.interest_career_alignment:
                career_areas = self.interest_career_alignment[interest][:3]  # Top 3 areas
                patterns_parts.append(f"  Aligned Career Areas: {', '.join(career_areas)}")
            
            # Add motivation insight
            if score >= 7:
                patterns_parts.append(f"  High intrinsic motivation for related activities")
            elif score >= 5:
                patterns_parts.append(f"  Good interest level - could develop into strong motivation")
            else:
    ...
    def _prepare_career_context(self, optional_data: Dict[str, Any]) -> str:
        ...
        if optional_data.get("geographical_preferences"):
            context_parts.append(f"Geographic Preferences: {optional_data['geographical_preferences']}")
        
        # Financial considerations
        if optional_data.get("financial_considerations"):
            context_parts.append(f"Financial Context: {optional_data['financial_considerations']}")
        
        # Family background and expectations
        if optional_data.get("family_background"):
            context_parts.append(f"Family Background: {optional_data['family_background']}")
        
        # Current career interests/aspirations
        if optional_data.get("career_aspirations"):
            context_parts.append(f"Current Career Interests: {optional_data['career_aspirations']}")
        
        return "\n".join(context_parts) if context_parts else "No specific contextual constraints identified"
...
"""
    ...
    def _calculate_aptitude_career_scores(self, dbda_scores: Dict[str, Any]) -> Dict[str, float]:
        """Calculate career compatibility scores based on aptitudes"""
        career_scores = {}
        
        # Map aptitudes to career clusters and calculate scores
        for career_cluster in ["Engineering", "Medicine", "Business", "Arts", "Research", "Technology"]:
            score = 0.0
            relevant_aptitudes = []
            ...
            career_scores[career_cluster] = round(score / len(relevant_aptitudes) if relevant_aptitudes else 0, 3)
        
        return career_scores
    
    def _calculate_interest_career_scores(self, cii_results: Dict[str, Any]) -> Dict[str, float]:
        """Calculate career motivation scores based on interests"""
        career_scores = {}
        
        # Map interests to career areas
        for career_area in ["Healthcare", "Technology", "Business", "Creative", "Social Service", "Research"]:
            score = 0.0
            relevant_interests = []
            ...
            career_scores[career_area] = round(score / len(relevant_interests) if relevant_interests else 0, 3)
        
        return career_scores
    
    def _analyze_stream_pathways(self, stream_recommendations: str, dbda_scores: Dict[str, Any], cii_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for stream in recommended_streams:
            if stream in self.stream_career_mapping:
                ...
                analysis["stream_career_alignment"][stream] = stream_info["primary_careers"][:5]
                analysis["pathway_feasibility"][stream] = self._assess_pathway_feasibility(stream, dbda_scores, cii_results)
        
        return analysis
    
    def _assess_pathway_feasibility(self, stream: str, dbda_scores: Dict[str, Any], cii_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if stream == "Science (PCM)":
            if dbda_scores.get("technical", 0) >= 7 and dbda_scores.get("computational", 0) >= 6:
                feasibility["high_feasibility"].append("Engineering and Technology careers")
            elif dbda_scores.get("technical", 0) >= 5:
                feasibility["moderate_feasibility"].append("Engineering with focused preparation")
            else:
                feasibility["requires_development"].append("Technical and mathematical skills")
        
        elif stream == "Science (PCB)":
            if dbda_scores.get("medical", 0) >= 7 and cii_results.get("medical", 0) >= 7:
                feasibility["high_feasibility"].append("Medical and Healthcare careers")
            elif dbda_scores.get("medical", 0) >= 5:
                feasibility["moderate_feasibility"].append("Healthcare with dedicated preparation")
            else:
                feasibility["requires_development"].append("Medical aptitude and interest")
        
        return feasibility
    
    def _calculate_career_readiness(self, dbda_scores: Dict[str, Any], cii_results: Dict[str, Any], optional_data: Dict[str, Any]) -> Dict[str, float]:
        """Calculate readiness scores for different career paths"""
        readiness_scores = {}
        
        # Calculate readiness for major career paths
        career_paths = ["Engineering", "Medicine", "Business", "Arts", "Research", "Technology"]
        ...
        for path in career_paths:
            ...
            if path == "Engineering":
                aptitude_boost = (dbda_scores.get("technical", 0) + dbda_scores.get("computational", 0)) / 20
            elif path == "Medicine":
                aptitude_boost = (dbda_scores.get("medical", 0) + dbda_scores.get("humanitarian", 0)) / 20
            elif path == "Business":
                aptitude_boost = (dbda_scores.get("administrative", 0) + dbda_scores.get("computational", 0)) / 20
            else:
                aptitude_boost = 0.1
            
            # Add interest component
            if path == "Engineering":
                interest_boost = (cii_results.get("technical", 0) + cii_results.get("computational", 0)) / 20
            elif path == "Medicine":
                interest_boost = (cii_results.get("medical", 0) + cii_results.get("humanitarian", 0)) / 20
            elif path == "Business":
                interest_boost = (cii_results.get("administrative", 0) + cii_results.get("clerical", 0)) / 20
            else:
                interest_boost = 0.1
            
            # Calculate final readiness
            readiness_scores[path] = round(min(1.0, base_score + aptitude_boost + interest_boost), 3)
        
        return readiness_scores
    
    def _get_top_career_matches(self, dbda_scores: Dict[str, Any], cii_results: Dict[str, Any], stream_recommendations: str) -> List[Dict[str, Any]]:
        """Get top career matches based on all factors"""
        career_matches = []
        
        # Calculate combined scores for major careers
        major_careers = [
            "Software Engineer", "Doctor", "Business Analyst", "Teacher", 
            "Researcher", "Designer", "Civil Servant", "Consultant"
        ]
        ...
        for career in major_careers:
            ...
            career_matches.append({
                ...
                "match_level": "High" if combined_score >= 0.7 else "Moderate" if combined_score >= 0.5 else "Low"
            })
    ...
    def _get_career_aptitude_score(self, career: str, dbda_scores: Dict[str, Any]) -> float:
    ...
    def _get_career_interest_score(self, career: str, cii_results: Dict[str, Any]) -> float:
