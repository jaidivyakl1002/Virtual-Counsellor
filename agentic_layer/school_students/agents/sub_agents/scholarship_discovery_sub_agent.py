...
class ScholarshipDiscoveryOutput(BaseModel):
    ...
    prioritized_scholarships: List[Dict[str, Any]] = Field(description="Scholarships ranked by suitability and probability")
    application_schedule: Dict[str, List[str]] = Field(description="Timeline for scholarship applications")
    eligibility_optimization: Dict[str, List[str]] = Field(description="Ways to improve eligibility")
    total_potential_funding: str = Field(description="Estimated total funding potential")
    success_strategies: List[str] = Field(description="Strategies to maximize scholarship success")
...
class ScholarshipDiscoverySubAgent:
    ...
    def __init__(self, llm_model):
        ...
        self.output_parser = JsonOutputParser(pydantic_object=ScholarshipDiscoveryOutput)
        ...
        self.prompt = PromptTemplate(
            input_variables=["student_profile", "academic_achievements", "financial_need", "career_pathway", "demographic_info"],
...
{format_instructions}"""
    ...
    def discover_scholarships(self, student_profile: str, academic_achievements: str,
                            financial_need: str, career_pathway: str, 
                            demographic_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result["scholarship_analytics"] = self._analyze_scholarship_landscape(
            result.get("prioritized_scholarships", []), 
            demographic_info
        )
        
        return result
    
    def _analyze_scholarship_landscape(self, scholarships: List[Dict], 
                                     demographic_info: Dict) -> Dict[str, Any]:
        ...
        if demo_info.get("gender") == "Female":
            analytics["demographic_advantages"].append("Girl child specific scholarships")
