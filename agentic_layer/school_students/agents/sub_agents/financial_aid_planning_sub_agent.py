...
class FinancialAidPlanningOutput(BaseModel):
    ...
    comprehensive_cost_analysis: Dict[str, Any] = Field(description="Detailed cost breakdown and projections")
    funding_strategy: Dict[str, List[str]] = Field(description="Multi-source funding approach")
    loan_optimization: Dict[str, Any] = Field(description="Education loan strategy and optimization")
    roi_analysis: Dict[str, Any] = Field(description="Return on investment analysis for education")
...
class FinancialAidPlanningSubAgent:
    ...
    def __init__(self, llm_model):
        ...
        self.output_parser = JsonOutputParser(pydantic_object=FinancialAidPlanningOutput)
        ...
        self.cost_benchmarks = {
            ...
            "Business": {
                ...
                "Government": {"tuition": 100000, "hostel": 50000, "misc": 30000},
                "Private_Premium": {"tuition": 1800000, "hostel": 150000, "misc": 80000}
            },
            "Liberal_Arts": {
        ...
        self.loan_parameters = {
            ...
            "interest_rates": {"government_bank": 8.5, "private_bank": 10.5, "nbfc": 12.0},
        ...
        self.prompt = PromptTemplate(
            input_variables=["student_profile", "college_costs", "family_income", "scholarship_potential", "loan_preferences"],
...
{format_instructions}"""
    ...
    def create_financial_plan(self, student_profile: str, college_costs: Dict[str, float],
                            family_income: str, scholarship_potential: str,
                            loan_preferences: str) -> Dict[str, Any]:
    ...
    def _calculate_financial_metrics(self, college_costs: Dict[str, float], 
                                   family_income: str, scholarship_potential: str) -> Dict[str, Any]:
        ...
        cost_to_income_ratio = (total_annual_cost * 4) / estimated_income  # 4-year program
        
        # Extract scholarship potential
        try:
            scholarship_amount = float(scholarship_potential.replace("₹", "").replace(",", "")) if "₹" in str(scholarship_potential) else 50000
        except:
            scholarship_amount = 50000
        
        net_annual_cost = max(0, total_annual_cost - scholarship_amount)
        loan_requirement = max(0, (net_annual_cost * 4) - (estimated_income * 0.2))  # Assuming 20% family contribution
        
        return {
            "financial_ratios": {
                "cost_to_income_ratio": round(cost_to_income_ratio, 2),
                "affordability_index": "High" if cost_to_income_ratio < 2 else "Medium" if cost_to_income_ratio < 4 else "Low"
            },
            "funding_breakdown": {
            ...
            "repayment_projections": {
                "estimated_monthly_emi": round(loan_requirement * 0.012, 0) if loan_requirement > 0 else 0,  # Rough EMI calculation
                "repayment_period": "10-15 years" if loan_requirement > 500000 else "5-10 years"
            }
        }
