...
class ParentalAlignmentOutput(BaseModel):
    ...
    alignment_assessment: Dict[str, str] = Field(description="Assessment of parent-student alignment")
    discussion_strategies: List[str] = Field(description="Strategies for family discussions")
    compromise_solutions: List[str] = Field(description="Potential compromise approaches")
    parent_education_points: List[str] = Field(description="Key points to educate parents about")
    potential_conflicts: List[str] = Field(description="Potential areas of conflict and how to address them")
...
class ParentalAlignmentSubAgent:
    ...
    def __init__(self, llm_model):
        ...
        self.prompt = PromptTemplate(
            input_variables=["student_preferences", "family_expectations", "assessment_results", "recommended_streams"],
...
{format_instructions}"""
    ...
    def assess_alignment(self, student_preferences: Dict, family_expectations: Dict, 
                        assessment_results: Dict, recommended_streams: List[Dict]) -> Dict[str, Any]:
        ...
        prompt_input = {
            ...
            } for stream in recommended_streams[:3]], indent=2),
