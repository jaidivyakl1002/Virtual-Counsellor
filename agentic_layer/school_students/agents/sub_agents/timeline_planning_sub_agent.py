...
class TimelinePlanningOutput(BaseModel):
    ...
    immediate_action_plan: List[str] = Field(description="Specific actions for next 1-3 months")
    monthly_review_schedule: Dict[str, List[str]] = Field(description="Monthly review and adjustment points")
    critical_deadlines: List[Dict[str, str]] = Field(description="Key deadlines and their importance")
    milestone_tracking: Dict[str, List[str]] = Field(description="How to track progress on key milestones")
    contingency_planning: List[str] = Field(description="Backup plans for potential issues")
...
class TimelinePlanningSubAgent:
    ...
    def __init__(self, llm_model):
        ...
        self.prompt = PromptTemplate(
            input_variables=["student_profile", "current_grade", "career_goals", "entrance_exams", "constraints"],
...
{format_instructions}"""
    ...
    def generate_timeline(self, student_profile: str, current_grade: str, 
                         career_goals: str, entrance_exams: List[str], 
        ...
        prompt_input = {
            ...
            "entrance_exams": ", ".join(entrance_exams) if entrance_exams else "To be determined",
