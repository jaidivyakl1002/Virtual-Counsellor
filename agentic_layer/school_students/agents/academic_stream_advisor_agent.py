...
from agentic_layer.school_students.agents.sub_agents.stream_decision_support_sub_agent import StreamDecisionSupportSubAgent
from agentic_layer.school_students.agents.sub_agents.parental_alignment_sub_agent import ParentalAlignmentSubAgent
...
class StreamRecommendation(BaseModel):
    ...
    primary_strengths_supporting: List[str] = Field(description="Key strengths that support this stream")
    interest_alignment: List[str] = Field(description="How interests align with this stream")
    career_pathways: List[str] = Field(description="Major career pathways from this stream")
    subject_requirements: List[str] = Field(description="Specific subjects required/recommended")
    challenges_to_consider: List[str] = Field(description="Potential challenges in this stream")
    success_predictors: List[str] = Field(description="Factors that predict success in this stream")

class StreamAdvisorOutput(BaseModel):
    """Structured output for academic stream advice"""
    executive_summary: str = Field(description="Brief overview of stream recommendations")
    recommended_streams: List[StreamRecommendation] = Field(description="Ranked stream recommendations")
    stream_comparison_matrix: Dict[str, Any] = Field(description="Comparative analysis of suitable streams")
    subject_wise_guidance: Dict[str, Any] = Field(description="Guidance for specific subjects within streams")
    parental_discussion_points: List[str] = Field(description="Key points to discuss with parents")
    decision_timeline: Dict[str, Any] = Field(description="Timeline for stream selection decision")
    backup_options: List[str] = Field(description="Alternative options if primary choices don't work")
...
class AcademicStreamAdvisorAgent(BaseAgent):
    ...
    def __init__(self, llm_model=None, config: Dict[str, Any] = None):
        ...
        self.stream_subjects = {
            ...
            "Commerce (Math)": {
                ...
                "optional_subjects": ["Computer Science", "Statistics", "Entrepreneurship"],
            ...
            "Arts/Humanities": {
                ...
                "optional_subjects": ["Psychology", "Sociology", "Literature", "Philosophy"],
        ...
        self.aptitude_stream_mapping = {
            "computational": {"Science (PCM)": 0.9, "Science (PCMB)": 0.8, "Commerce (Math)": 0.8},
            "experimental": {"Science (PCB)": 0.9, "Science (PCMB)": 0.9, "Science (PCM)": 0.7},
            "technical": {"Science (PCM)": 0.9, "Science (PCMB)": 0.7, "Vocational": 0.8},
            "medical": {"Science (PCB)": 0.95, "Science (PCMB)": 0.9},
            "administrative": {"Commerce (Math)": 0.8, "Commerce (No Math)": 0.8, "Arts/Humanities": 0.6},
            "humanitarian": {"Arts/Humanities": 0.9, "Commerce (No Math)": 0.6, "Science (PCB)": 0.5},
            "educational": {"Arts/Humanities": 0.8, "Science (PCB)": 0.6, "Commerce (No Math)": 0.5},
            "creative": {"Arts/Humanities": 0.8, "Commerce (No Math)": 0.5, "Vocational": 0.7},
            "nature": {"Science (PCB)": 0.8, "Science (PCMB)": 0.7, "Arts/Humanities": 0.6}
        }
    ...
    def _define_optional_inputs(self) -> List[str]:
        ...
        return [
            "current_grade", "academic_performance", "subject_preferences", 
            "family_preferences", "career_aspirations", "geographical_constraints",
    ...
    def _initialize_agent(self):
        ...
        self.advisory_prompt = PromptTemplate(
            input_variables=[
                "student_profile", "test_interpretation", "aptitude_strengths", 
                "interest_patterns", "stream_options", "contextual_factors"
            ],
...
Remember: This recommendation will significantly impact the student's academic journey. Provide balanced, evidence-based advice that considers both potential and practical constraints."""
    ...
    def _process_core_logic(self, validated_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Add processing note
        self._add_processing_note("Starting academic stream analysis and recommendation")
        ...
        try:
            ...
            result["computational_analysis"] = {
                "aptitude_stream_compatibility": self._calculate_stream_compatibility(dbda_scores),
                "interest_stream_alignment": self._calculate_interest_alignment(cii_results),
                "combined_suitability_scores": self._calculate_combined_suitability(dbda_scores, cii_results),
                "top_3_recommended_streams": self._get_top_stream_recommendations(dbda_scores, cii_results)
            }
            
            family_context = {
                "family_preferences": optional_data.get("family_preferences"),
                "financial_considerations": optional_data.get("financial_considerations"),
            ...
            decision_support_result = self.decision_support_agent.generate_support(
                ...
                academic_performance=optional_data.get("academic_performance", "Not specified")
            )
            ...
            student_preferences = {
                ...
                "assessment_based_strengths": self._get_top_aptitudes_and_interests(dbda_scores, cii_results)
            }
            
            family_expectations = {
                "family_preferences": optional_data.get("family_preferences"),
                "family_background": optional_data.get("family_background"),
                "cultural_context": optional_data.get("geographical_constraints")  # Can indicate cultural region
            }
            ...
            result["parental_discussion_points"] = alignment_result.get("discussion_strategies", [])
    ...
    def _get_top_aptitudes_and_interests(self, dbda_scores: Dict[str, Any], cii_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "top_aptitudes": [{"domain": apt, "score": score} for apt, score in top_aptitudes],
            "top_interests": [{"domain": interest, "score": score} for interest, score in top_interests]
        }
    ...
    def _prepare_student_profile(self, optional_data: Dict[str, Any]) -> str:
        ...
        if optional_data.get("career_aspirations"):
            profile_parts.append(f"Career Aspirations: {optional_data['career_aspirations']}")
        
        return "\n".join(profile_parts) if profile_parts else "Basic student profile available"
    
    def _extract_test_interpretation(self, interpreter_result) -> str:
        """Extract key insights from test score interpreter agent"""
        if not interpreter_result or interpreter_result.status != ProcessingStatus.COMPLETED:
        ...
        if output_data.get("executive_summary"):
            interpretation_parts.append(f"Assessment Overview: {output_data['executive_summary']}")
        ...
        if output_data.get("aptitude_interest_alignment"):
            ...
            interpretation_parts.append(f"Aptitude-Interest Alignment: {alignment_info}")
        
        return "\n".join(interpretation_parts) if interpretation_parts else "Limited test interpretation available"
    ...
    def _identify_aptitude_strengths(self, dbda_scores: Dict[str, Any]) -> str:
        ...
        if not strengths:
            ...
            sorted_scores = sorted(dbda_scores.items(), key=lambda x: x[1], reverse=True)
            top_3 = sorted_scores[:3]
            strength_lines = [f"Relative Strength - {domain.replace('_', ' ').title()}: {score}/10" 
                            for domain, score in top_3]
        else:
            strength_lines = [f"{domain.replace('_', ' ').title()}: {score}/10 (Strong)" 
                            for domain, score in strengths]
        
        return "Identified Aptitude Strengths:\n" + "\n".join(f"- {line}" for line in strength_lines)
    ...
    def _extract_interest_patterns(self, cii_results: Dict[str, Any]) -> str:
        ...
        interests = [(domain, score) for domain, score in cii_results.items() if score >= 6]
        interests.sort(key=lambda x: x[1], reverse=True)
        
        if not interests:
            # If no clear interests, identify relative interests
            sorted_interests = sorted(cii_results.items(), key=lambda x: x[1], reverse=True)
            top_3 = sorted_interests[:3]
            interest_lines = [f"Moderate Interest - {domain.replace('_', ' ').title()}: {score}/10" 
                            for domain, score in top_3]
        else:
            interest_lines = [f"{domain.replace('_', ' ').title()}: {score}/10 (High Interest)" 
    ...
    def _format_stream_options(self) -> str:
        ...
        for stream, details in self.stream_subjects.items():
            ...
            stream_lines.append(f"  Core Subjects: {', '.join(details['core_subjects'])}")
            stream_lines.append(f"  Optional Subjects: {', '.join(details['optional_subjects'])}")
    ...
    def _prepare_contextual_factors(self, optional_data: Dict[str, Any]) -> str:
        ...
        if optional_data.get("financial_considerations"):
            factors.append(f"Financial Considerations: {optional_data['financial_considerations']}")
        
        # Geographic constraints
        if optional_data.get("geographical_constraints"):
            factors.append(f"Geographic Constraints: {optional_data['geographical_constraints']}")
        
        # Academic performance context
        if optional_data.get("academic_performance"):
            factors.append(f"Current Academic Performance: {optional_data['academic_performance']}")
        
        return "\n".join(factors) if factors else "No specific contextual constraints mentioned"
    
    def _calculate_stream_compatibility(self, dbda_scores: Dict[str, Any]) -> Dict[str, float]:
    ...
    def _calculate_interest_alignment(self, cii_results: Dict[str, Any]) -> Dict[str, float]:
    ...
    def _calculate_combined_suitability(self, dbda_scores: Dict[str, Any], cii_results: Dict[str, Any]) -> Dict[str, float]:
        ...
        for stream in self.stream_subjects.keys():
            ...
            combined = (aptitude_scores.get(stream, 0) * 0.6) + (interest_scores.get(stream, 0) * 0.4)
            combined_scores[stream] = round(combined, 3)
        
        return combined_scores
    
    def _get_top_stream_recommendations(self, dbda_scores: Dict[str, Any], cii_results: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        sorted_streams = sorted(combined_scores.items(), key=lambda x: x[1], reverse=True)
