...
from agentic_layer.school_students.agents.sub_agents.timeline_planning_sub_agent import TimelinePlanningSubAgent
from agentic_layer.school_students.agents.sub_agents.resource_planning_sub_agent import ResourcePlanningSubAgent
...
class GradeMilestone(BaseModel):
    ...
    academic_focus: List[str] = Field(description="Key academic priorities for this grade")
    subject_priorities: Dict[str, str] = Field(description="Subject-wise focus areas")
    entrance_exam_preparation: List[str] = Field(description="Entrance exam preparation activities")
    extracurricular_recommendations: List[str] = Field(description="Recommended extracurricular activities")
    skill_development_goals: List[str] = Field(description="Skills to develop during this grade")
    assessment_milestones: List[str] = Field(description="Key assessments and benchmarks")
    college_preparation_activities: List[str] = Field(description="College/career preparation activities")
    timeline_considerations: Dict[str, Any] = Field(description="Important timing considerations")
...
class EntranceExamStrategy(BaseModel):
    ...
    subject_priorities: List[str] = Field(description="Subject-wise preparation priorities")
    preparation_strategy: List[str] = Field(description="Preparation approach and methods")
    coaching_recommendations: str = Field(description="Coaching requirements and recommendations")
    mock_test_schedule: List[str] = Field(description="Mock test and practice schedule")
    difficulty_assessment: str = Field(description="Expected difficulty and competition level")
...
class HigherEducationPathway(BaseModel):
    ...
    top_institutions: List[str] = Field(description="Top colleges/universities for this path")
    specialization_options: List[str] = Field(description="Available specializations")
    career_outcomes: List[str] = Field(description="Career opportunities after graduation")
    financial_considerations: Dict[str, Any] = Field(description="Cost and financial aid information")
...
class EducationalRoadmapPlannerOutput(BaseModel):
    ...
    grade_wise_milestones: List[GradeMilestone] = Field(description="Detailed grade-wise planning")
    entrance_exam_strategies: List[EntranceExamStrategy] = Field(description="Entrance exam preparation plans")
    higher_education_pathways: List[HigherEducationPathway] = Field(description="Post-12th education options")
    subject_selection_guidance: Dict[str, Any] = Field(description="Subject and elective selection advice")
    timeline_overview: Dict[str, Any] = Field(description="Overall timeline and critical decision points")
    backup_plans: List[str] = Field(description="Alternative pathways and contingency plans")
    resource_requirements: Dict[str, Any] = Field(description="Resources needed (financial, time, support)")
    progress_monitoring: Dict[str, Any] = Field(description="How to track progress and adjust plans")
...
class EducationalRoadmapPlannerAgent(BaseAgent):
    ...
    def __init__(self, llm_model=None, config: Dict[str, Any] = None):
        ...
        self.stream_exam_mapping = {
            ...
            "Science (PCMB)": {
                ...
                "specialized_exams": ["Dual degree entrance exams", "Research entrance tests"],
                "preparation_subjects": ["Mathematics", "Physics", "Chemistry", "Biology"],
                "coaching_recommended": "Recommended for competitive preparation"
            },
            "Commerce (Math)": {
                "primary_exams": ["CA Foundation", "CMA Foundation", "Company Secretary"],
            ...
            "Arts/Humanities": {
                ...
                "specialized_exams": ["Mass Communication", "Social Work Entrance", "Education Entrance"],
                "preparation_subjects": ["History", "Political Science", "Geography", "English"],
        ...
        self.grade_focus_mapping = {
            "9": {
                ...
                "skill_focus": ["Study habits", "Time management", "Basic research skills"],
                "preparation_activities": ["Explore interests", "Build subject fundamentals", "Develop reading habits"],
            ...
            "10": {
                ...
                "preparation_activities": ["Stream selection", "Career counseling", "Board exam preparation"],
            ...
            "11": {
                ...
                "skill_focus": ["Specialized subject knowledge", "Entrance exam awareness", "Advanced study methods"],
                "preparation_activities": ["Entrance exam foundation", "Subject specialization", "College research"],
            ...
            "12": {
                ...
                "skill_focus": ["Intensive preparation", "Stress management", "Decision making"],
                "preparation_activities": ["Final preparations", "College applications", "Scholarship applications"],
        ...
        self.education_pathways = {
            "Engineering": {
                ...
                "career_outcomes": ["Software Engineer", "Design Engineer", "Research Engineer", "Product Manager"]
            },
            "Medicine": {
                "degrees": ["MBBS", "BDS", "BAMS", "BHMS"],
                "top_institutions": ["AIIMS", "CMCs", "JIPMER", "State Medical Colleges"],
                "specializations": ["Internal Medicine", "Surgery", "Pediatrics", "Radiology"],
                "entry_exams": ["NEET UG"],
                "duration": "5.5 years (MBBS), 5 years (BDS)",
                "career_outcomes": ["Doctor", "Surgeon", "Specialist", "Medical Researcher"]
            },
            "Business": {
            ...
            "Law": {
                ...
                "career_outcomes": ["Lawyer", "Judge", "Legal Advisor", "Civil Servant"]
            }
        }
    ...
    def _define_optional_inputs(self) -> List[str]:
        ...
        return [
            "current_grade", "recommended_streams", "career_interests",  # These come from user_data or previous agents
            "dbda_scores", "cii_results", "academic_performance", "family_preferences",
            "financial_considerations", "geographical_preferences", "extracurricular_activities"
        ]
    ...
    def _initialize_agent(self):
        ...
        self.output_parser = JsonOutputParser(pydantic_object=EducationalRoadmapPlannerOutput)
        
        # Create the main roadmap planning prompt
        self.planning_prompt = PromptTemplate(
            input_variables=[
                "student_profile", "current_academic_status", "stream_recommendations", 
                "career_goals", "assessment_insights", "contextual_factors", "planning_framework"
            ],
...
Remember: This roadmap will guide critical educational decisions. Provide practical, achievable plans that balance ambition with realism, considering the student's specific circumstances and the competitive nature of Indian education."""
    ...
    def _process_core_logic(self, validated_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "academic_stream_advisor" in previous_outputs:
            ...
            if stream_result.status == ProcessingStatus.COMPLETED:
                ...
                recommended_streams = [stream.get("stream_type") for stream in streams if isinstance(stream, dict)]
        
        if not recommended_streams:
            recommended_streams = ["Science (PCM)", "Science (PCB)", "Commerce (Math)"]  # Default fallback
        ...
        if "career_pathway_explorer" in previous_outputs:
            ...
            if career_result.status == ProcessingStatus.COMPLETED:
                careers = career_result.output_data.get("recommended_career_pathways", [])
                career_interests = [career.get("career_title") for career in careers if isinstance(career, dict)]
        
        if not career_interests:
            career_interests = ["Engineering", "Medicine", "Business"]  # Default fallback
        ...
        current_academic_status = self._assess_current_academic_status(current_grade, optional_data)
        stream_recommendations = self._extract_stream_recommendations(stream_advisor_result, recommended_streams)
        career_goals = self._extract_career_goals(career_explorer_result, career_interests)
        assessment_insights = self._extract_assessment_insights(test_interpreter_result, optional_data)
        ...
        try:
            ...
            result["roadmap_metadata"] = {
                ...
                "based_on_assessments": self._list_assessment_sources(test_interpreter_result, stream_advisor_result, career_explorer_result)
            }
            
            # Add computational analysis
            result["computational_analysis"] = {
                "timeline_analysis": self._calculate_timeline_metrics(current_grade),
                "entrance_exam_priorities": self._prioritize_entrance_exams(stream_recommendations, career_goals),
                "resource_estimation": self._estimate_resource_requirements(recommended_streams, current_grade),
                "success_probability_factors": self._analyze_success_factors(optional_data, current_grade),
                "critical_decision_points": self._identify_critical_decisions(current_grade)
            }

            relevant_exams = self._extract_relevant_entrance_exams(result["entrance_exam_strategies"])
            constraints = {
            ...
            self._add_processing_note("Dynamic timeline planning generated successfully")
            ...
            resource_result = self.resource_planning_agent.generate_resource_plan(
                ...
                financial_context=optional_data.get("financial_considerations", "Middle-class family"),
                location_context=optional_data.get("geographical_preferences", "Urban India"),
                grade_timeline=f"Grade {current_grade} to Grade 12 + Higher Education"
            )
            
            result["resource_planning"] = resource_result
            self._add_processing_note("Dynamic resource planning generated successfully")

            # Replace the hardcoded practical_guidance with this dynamic version:
            result["practical_guidance"] = {
                "immediate_actions": result["timeline_planning"]["immediate_action_plan"],
                "resource_optimization": result["resource_planning"]["alternative_resource_options"],
                "monitoring_schedule": result["timeline_planning"]["monthly_review_schedule"],
                "support_system": result["resource_planning"]["support_system_requirements"],
                "financial_planning": result["resource_planning"]["financial_planning"]
            }
            
            self._add_processing_note("Educational roadmap planning completed successfully")
            return result
    ...
    def _extract_relevant_entrance_exams(self, entrance_strategies: List[Dict]) -> List[str]:
    ...
    def _summarize_career_pathway(self, career_explorer_result) -> str:
        ...
        if not career_explorer_result or career_explorer_result.status != ProcessingStatus.COMPLETED:
    ...
    def _prepare_student_profile(self, optional_data: Dict[str, Any], current_grade: str) -> str:
        ...
        if optional_data.get("cii_results"):
            top_interests = sorted(optional_data["cii_results"].items(), key=lambda x: x[1], reverse=True)[:2]
            interest_summary = ", ".join([interest.replace('_', ' ').title() for interest, _ in top_interests])
            profile_parts.append(f"Top Interest Areas: {interest_summary}")
        
        return "\n".join(profile_parts) if profile_parts else f"Student currently in {current_grade}"
    
    def _assess_current_academic_status(self, current_grade: str, optional_data: Dict[str, Any]) -> str:
        ...
        if grade_num == 9:
            status_parts.append("Foundation building phase - focusing on concept clarity and study habits")
            status_parts.append("Stream selection awareness stage - exploring different academic paths")
        elif grade_num == 10:
            status_parts.append("Board exam preparation year - crucial for academic foundation")
            status_parts.append("Stream decision timeline - must finalize by end of academic year")
        elif grade_num == 11:
            status_parts.append("Stream specialization year - intensive subject focus required")
            status_parts.append("Entrance exam preparation foundation - building competitive exam readiness")
        elif grade_num == 12:
            status_parts.append("Final preparation year - board exams and entrance exams")
        ...
        if "excellent" in performance or "outstanding" in performance:
            ...
            status_parts.append("Solid academic foundation with room for strategic improvement")
        elif "average" in performance:
            status_parts.append("Needs focused effort to improve competitive positioning")
        elif performance and ("below" in performance or "weak" in performance):
            status_parts.append("Requires intensive support and structured improvement plan")
        
        return "\n".join(status_parts)
    
    def _extract_stream_recommendations(self, stream_advisor_result, recommended_streams) -> str:
        """Extract and format stream recommendations from previous agent"""
        if stream_advisor_result and stream_advisor_result.status == ProcessingStatus.COMPLETED:
            output_data = stream_advisor_result.output_data
            
            stream_parts = []
            if output_data.get("executive_summary"):
                stream_parts.append(f"Stream Advisory Summary: {output_data['executive_summary']}")
                    ...
                    for i, stream in enumerate(streams[:3], 1):
                        if isinstance(stream, dict):
                            ...
                            if stream.get("career_pathways"):
                                pathways = stream["career_pathways"][:3]  # Top 3 pathways
                                stream_parts.append(f"   Key Career Paths: {', '.join(pathways)}")
    ...
    def _extract_career_goals(self, career_explorer_result, career_interests) -> str:
        ...
        if career_explorer_result and career_explorer_result.status == ProcessingStatus.COMPLETED:
            output_data = career_explorer_result.output_data
            
            career_parts = []
            if output_data.get("executive_summary"):
                career_parts.append(f"Career Exploration Summary: {output_data['executive_summary']}")
                ...
                if isinstance(pathways, list):
                    for i, pathway in enumerate(pathways[:3], 1):
                        if isinstance(pathway, dict):
                            ...
                            suitability = pathway.get("suitability_score", "Not specified")
                            career_parts.append(f"{i}. {career_title} (Suitability: {suitability})")
                            
                            # Add educational requirements if available
                            if pathway.get("educational_requirements"):
                                edu_reqs = pathway["educational_requirements"][:2]  # Top 2 requirements
                                career_parts.append(f"   Education: {', '.join(edu_reqs)}")
    ...
    def _extract_assessment_insights(self, test_interpreter_result, optional_data: Dict[str, Any]) -> str:
        """Extract key insights from assessment interpretation"""
        if test_interpreter_result and test_interpreter_result.status == ProcessingStatus.COMPLETED:
            output_data = test_interpreter_result.output_data
            
            insight_parts = []
            if output_data.get("executive_summary"):
                insight_parts.append(f"Assessment Overview: {output_data['executive_summary']}")
            ...
            if optional_data.get("dbda_scores"):
                top_aptitudes = sorted(optional_data["dbda_scores"].items(), key=lambda x: x[1], reverse=True)[:2]
                aptitudes = [apt.replace('_', ' ').title() for apt, score in top_aptitudes]
                insight_parts.append(f"Top Aptitudes: {', '.join(aptitudes)}")
            
            if optional_data.get("cii_results"):
                top_interests = sorted(optional_data["cii_results"].items(), key=lambda x: x[1], reverse=True)[:2]
                interests = [interest.replace('_', ' ').title() for interest, score in top_interests]
                insight_parts.append(f"Top Interests: {', '.join(interests)}")
            
            return "\n".join(insight_parts) if insight_parts else "Assessment insights being analyzed"
    ...
    def _prepare_contextual_factors(self, optional_data: Dict[str, Any]) -> str:
        ...
        if optional_data.get("financial_considerations"):
            factors.append(f"Financial Context: {optional_data['financial_considerations']}")
        
        # Geographic constraints
        if optional_data.get("geographical_preferences"):
            factors.append(f"Geographic Preferences: {optional_data['geographical_preferences']}")
        
        # Current extracurricular engagement
        if optional_data.get("extracurricular_activities"):
            factors.append(f"Current Activities: {optional_data['extracurricular_activities']}")
        
        return "\n".join(factors) if factors else "No specific contextual constraints identified"
    ...
    def _create_planning_framework(self, current_grade: str) -> str:
        ...
        for grade in range(grade_num, 13):  # From current grade to 12th
            if str(grade) in self.grade_focus_mapping:
                ...
                framework_parts.append(f"- Academic Priority: {grade_info['academic_priority']}")
                framework_parts.append(f"- Skill Focus: {', '.join(grade_info['skill_focus'])}")
                framework_parts.append(f"- Key Activities: {', '.join(grade_info['preparation_activities'])}")
    ...
    def _list_assessment_sources(self, test_result, stream_result, career_result) -> List[str]:
    ...
    def _calculate_timeline_metrics(self, current_grade: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "preparation_timeline": {
                "stream_selection": "End of Grade 10" if grade_num <= 10 else "Already decided",
                "entrance_exam_prep": "Grade 11-12" if grade_num <= 11 else "Grade 12",
                "college_applications": "Grade 12" if grade_num <= 12 else "Current year"
            }
        }
    
    def _prioritize_entrance_exams(self, stream_recommendations: str, career_goals: str) -> List[Dict[str, Any]]:
        ...
        if "Science (PCM)" in stream_recommendations or "Engineering" in career_goals:
            priorities.append({
                ...
                "success_factors": ["Math proficiency", "Physics concepts", "Chemistry fundamentals"]
            })
            priorities.append({
                "exam": "JEE Advanced", 
                "priority": "Medium",
                "preparation_time": "24+ months",
                "success_factors": ["JEE Main qualification", "Advanced problem solving"]
            })
        ...
        if "Science (PCB)" in stream_recommendations or "Medicine" in career_goals:
            priorities.append({
                ...
                "priority": "High", 
                "preparation_time": "18-24 months",
                "success_factors": ["Biology mastery", "Chemistry concepts", "Physics basics"]
            })
        ...
        if "Commerce" in stream_recommendations or "Business" in career_goals:
            priorities.append({
                ...
                "success_factors": ["Quantitative aptitude", "Verbal ability", "Logical reasoning"]
            })
        
        return priorities
    
    def _estimate_resource_requirements(self, recommended_streams: str, current_grade: str) -> Dict[str, Any]:
    ...
    def _analyze_success_factors(self, optional_data: Dict[str, Any], current_grade: str) -> List[str]:
        ...
        if "excellent" in performance:
            success_factors.append("Strong academic foundation provides competitive advantage")
        ...
        if optional_data.get("extracurricular_activities"):
            success_factors.append("Active extracurricular engagement shows well-rounded development")
        ...
        grade_num = self._extract_grade_number(current_grade)
        if grade_num <= 10:
            success_factors.append("Ample time for strategic preparation and skill building")
        elif grade_num == 11:
            success_factors.append("Adequate time for focused preparation with proper planning")
        else:
