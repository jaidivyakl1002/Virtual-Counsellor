...
from agentic_layer.school_students.agents.sub_agents.college_matching_sub_agent import CollegeMatchingSubAgent
from agentic_layer.school_students.agents.sub_agents.scholarship_discovery_sub_agent import ScholarshipDiscoverySubAgent
from agentic_layer.school_students.agents.sub_agents.financial_aid_planning_sub_agent import FinancialAidPlanningSubAgent
...
class CollegeRecommendation(BaseModel):
    ...
    college_type: str = Field(description="Type of institution (IIT, NIT, Private, etc.)")
    ...
    admission_requirements: Dict[str, Any] = Field(description="Specific admission criteria")
    fees_structure: Dict[str, Any] = Field(description="Detailed fee information")
    ranking_information: Dict[str, Any] = Field(description="Rankings and reputation metrics")
    campus_facilities: List[str] = Field(description="Key campus facilities and infrastructure")
    placement_statistics: Dict[str, Any] = Field(description="Placement and career outcomes")
    pros_and_cons: Dict[str, List[str]] = Field(description="Advantages and disadvantages")
    suitability_score: float = Field(description="Match score for this student (0-1)")
    application_timeline: Dict[str, str] = Field(description="Key application deadlines")
...
class ScholarshipOpportunity(BaseModel):
    ...
    eligibility_criteria: List[str] = Field(description="Detailed eligibility requirements")
    ...
    required_documents: List[str] = Field(description="Documents needed for application")
    ...
    renewal_conditions: List[str] = Field(description="Conditions for scholarship renewal")
    success_tips: List[str] = Field(description="Tips for successful application")
    compatibility_score: float = Field(description="How well student matches criteria (0-1)")
...
class FinancialAidPlan(BaseModel):
    ...
    timeline_based_planning: Dict[str, Any] = Field(description="Year-wise financial planning")
    cost_optimization_strategies: List[str] = Field(description="Ways to reduce educational costs")
...
class CollegeScholarshipNavigatorOutput(BaseModel):
    ...
    executive_summary: str = Field(description="Overview of college and financial aid guidance")
    recommended_colleges: List[CollegeRecommendation] = Field(description="Prioritized college recommendations")
    scholarship_opportunities: List[ScholarshipOpportunity] = Field(description="Relevant scholarship options")
    financial_aid_planning: FinancialAidPlan = Field(description="Comprehensive financial planning")
    application_strategy: Dict[str, Any] = Field(description="Strategic approach to applications")
    geographic_analysis: Dict[str, Any] = Field(description="Location-based considerations")
    backup_options: Dict[str, Any] = Field(description="Alternative plans and safety options")
    decision_framework: Dict[str, Any] = Field(description="How to make final decisions")
    timeline_coordination: Dict[str, Any] = Field(description="Coordinating all applications and deadlines")
...
class CollegeScholarshipNavigatorAgent(BaseAgent):
    ...
    def __init__(self, llm_model=None, config: Dict[str, Any] = None):
        ...
        self.entrance_exam_colleges = {
            "JEE Main": {
                "top_tier": ["IIT Delhi", "IIT Bombay", "IIT Madras", "IIT Kanpur", "IIT Kharagpur"],
                "second_tier": ["NIT Trichy", "NIT Warangal", "NIT Surathkal", "IIIT Hyderabad"],
                "private_options": ["BITS Pilani", "VIT Vellore", "Manipal Institute", "SRM University"],
                "specializations": ["Computer Science", "Electronics", "Mechanical", "Civil"]
            },
            "NEET UG": {
                "top_tier": ["AIIMS Delhi", "CMC Vellore", "JIPMER Puducherry", "KGMU Lucknow"],
                "government": ["State Medical Colleges", "Central Universities", "Military Medical Colleges"],
                "private_options": ["Kasturba Medical College", "JSS Medical College", "Amrita Medical College"],
                "specializations": ["General Medicine", "Dental", "Ayurveda", "Homeopathy"]
            },
            "CLAT": {
                "top_tier": ["NLSIU Bangalore", "NALSAR Hyderabad", "WB NUJS Kolkata", "NLIU Bhopal"],
                "government": ["DU Faculty of Law", "BHU Law", "Jamia Law", "Aligarh Law"],
                "private_options": ["Jindal Global Law School", "Symbiosis Law School", "Christ University Law"],
                "specializations": ["Corporate Law", "Criminal Law", "Constitutional Law", "International Law"]
            },
            "CAT": {
                "top_tier": ["IIM Ahmedabad", "IIM Bangalore", "IIM Calcutta", "IIM Lucknow"],
                "government": ["FMS Delhi", "DoMS IIT Delhi", "SJMSOM IIT Bombay"],
                "private_options": ["XLRI Jamshedpur", "ISB Hyderabad", "MDI Gurgaon", "SPJIMR Mumbai"],
                "specializations": ["Finance", "Marketing", "Operations", "HR", "Analytics"]
            }
        }
        ...
        self.scholarship_mapping = {
            "merit_based": {
                "academic_excellence": ["Kishore Vaigyanik Protsahan Yojana", "National Talent Search Examination"],
                "entrance_based": ["JEE Merit Scholarships", "NEET Merit Scholarships"],
                "university_specific": ["Institute Merit Scholarships", "Endowment Scholarships"]
            },
            "need_based": {
                "government": ["Post Matric Scholarships", "Pre Matric Scholarships", "Central Sector Scholarships"],
                "private": ["Tata Scholarships", "Reliance Foundation Scholarships", "Azim Premji Scholarships"],
                "institutional": ["Fee Waiver Programs", "Educational Loan Subsidies"]
            },
            "category_specific": {
                "sc_st": ["National Fellowship for SC/ST", "Post Matric Scholarship SC/ST"],
                "obc": ["Post Matric Scholarship OBC", "Central Sector Scholarship OBC"],
                "minority": ["Maulana Azad National Fellowship", "Minority Scholarship Schemes"]
            }
        }
    ...
    def _define_optional_inputs(self) -> List[str]:
        ...
        return [
            "career_goals", "educational_pathway", "academic_timeline",
            "dbda_scores", "cii_results", "current_grade",
            "academic_performance", "family_preferences", "financial_considerations",
            "geographical_preferences", "entrance_exam_results", "extracurricular_activities",
    ...
    def _initialize_agent(self):
        ...
        self.output_parser = JsonOutputParser(pydantic_object=CollegeScholarshipNavigatorOutput)
        
        # Create the main navigation prompt
        self.navigation_prompt = PromptTemplate(
            input_variables=[
                "student_profile", "career_pathway", "educational_timeline", "financial_context",
                "geographic_preferences", "academic_achievements", "assessment_insights", "market_context"
            ],
...
Remember: This guidance will influence major life decisions and financial commitments. Provide thorough, realistic recommendations that balance aspiration with practical constraints."""
    ...
    def _process_core_logic(self, validated_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "career_pathway_explorer" in previous_outputs:
            ...
            if career_result.status == ProcessingStatus.COMPLETED:
                ...
                career_goals = [career.get("career_title") for career in careers if isinstance(career, dict)]
        ...
        if "educational_roadmap_planner" in previous_outputs:
            ...
            if roadmap_result.status == ProcessingStatus.COMPLETED:
                pathways = roadmap_result.output_data.get("higher_education_pathways", [])
                educational_pathway = [pathway.get("degree_type") for pathway in pathways if isinstance(pathway, dict)]
        ...
        if not academic_timeline:
            academic_timeline = {"current_focus": "College preparation", "timeline": "Grade 12 to Higher Education"}
        ...
        career_pathway = self._extract_career_pathway(career_explorer_result, career_goals)
        educational_timeline = self._extract_educational_timeline(roadmap_planner_result, academic_timeline)
        ...
        academic_achievements = self._assess_academic_achievements(optional_data, previous_outputs)
        assessment_insights = self._consolidate_assessment_insights(test_interpreter_result, stream_advisor_result)
        market_context = self._prepare_market_context(career_pathway)
        
        # Add processing note
        self._add_processing_note("Starting comprehensive college and scholarship navigation")
        ...
        try:
            ...
            self._add_processing_note("Dynamic scholarship discovery completed successfully")
            
            financial_aid_result = self.financial_aid_planning_agent.create_financial_plan(
                student_profile=student_profile,
                college_costs=self._estimate_college_costs(result["recommended_colleges"]),
            ...
            self._add_processing_note("Dynamic financial planning completed successfully")
            
            # Add practical guidance
            result["practical_guidance"] = {
                "immediate_actions": result["dynamic_college_matching"]["next_steps"],
                "scholarship_application_timeline": result["dynamic_scholarship_discovery"]["application_schedule"],
                "financial_preparation": result["dynamic_financial_planning"]["preparation_checklist"],
                "decision_support": self._create_decision_support_framework(result),
                "family_discussion_points": self._generate_family_discussion_points(result)
            }
            
            self._add_processing_note("College and scholarship navigation completed successfully")
            return result
    ...
    def _prepare_student_profile(self, optional_data: Dict[str, Any], previous_outputs: Dict[str, Any]) -> str:
        ...
        if previous_outputs:
            if "academic_stream_advisor" in previous_outputs:
                ...
                if stream_result.status == ProcessingStatus.COMPLETED:
                    profile_parts.append("Academic stream guidance completed - preferences identified")
        
        return "\n".join(profile_parts) if profile_parts else "Student profile under development"
    ...
    def _extract_career_pathway(self, career_explorer_result, career_goals) -> str:
        ...
        if career_explorer_result and career_explorer_result.status == ProcessingStatus.COMPLETED:
            ...
            if output_data.get("executive_summary"):
                career_parts.append(f"Career Analysis: {output_data['executive_summary']}")
            ...
            if output_data.get("recommended_career_pathways"):
                ...
                if isinstance(pathways, list) and len(pathways) > 0:
                    ...
                    if isinstance(top_pathway, dict):
                        career_title = top_pathway.get("career_title", "Primary career path")
                        career_field = top_pathway.get("career_field", "")
                        career_parts.append(f"Primary Career Goal: {career_title} in {career_field}")
            
            return "\n".join(career_parts)
        else:
            return f"Career Goals: {career_goals}"
    
    def _extract_educational_timeline(self, roadmap_planner_result, academic_timeline) -> str:
        """Extract educational timeline from roadmap planning"""
        if roadmap_planner_result and roadmap_planner_result.status == ProcessingStatus.COMPLETED:
            output_data = roadmap_planner_result.output_data
            
            timeline_parts = []
            if output_data.get("executive_summary"):
                timeline_parts.append(f"Educational Roadmap: {output_data['executive_summary']}")
                ...
                if isinstance(timeline_overview, dict):
                    for key, value in timeline_overview.items():
                        if isinstance(value, str) and len(value) < 200:
                            timeline_parts.append(f"{key.replace('_', ' ').title()}: {value}")
    ...
    def _prepare_financial_context(self, optional_data: Dict[str, Any]) -> str:
        ...
        if optional_data.get("financial_considerations"):
            financial_parts.append(f"Financial Situation: {optional_data['financial_considerations']}")
        
        if optional_data.get("family_preferences"):
            preferences = optional_data["family_preferences"]
            if "budget" in str(preferences).lower() or "cost" in str(preferences).lower():
                financial_parts.append(f"Family Preferences: {preferences}")
        
        return "\n".join(financial_parts) if financial_parts else "Middle-class family seeking cost-effective options"
    ...
    def _extract_geographic_preferences(self, optional_data: Dict[str, Any]) -> str:
        ...
        if optional_data.get("geographical_preferences"):
            geo_parts.append(f"Location Preferences: {optional_data['geographical_preferences']}")
        
        if optional_data.get("family_preferences"):
            preferences = optional_data["family_preferences"]
            if any(word in str(preferences).lower() for word in ["local", "nearby", "home", "city", "state"]):
                geo_parts.append(f"Family Location Preferences: {preferences}")
        
        return "\n".join(geo_parts) if geo_parts else "Open to various locations based on quality and opportunities"
    
    def _assess_academic_achievements(self, optional_data: Dict[str, Any], previous_outputs: Dict[str, Any]) -> str:
        """Assess academic achievements and competitive positioning"""
        achievement_parts = []
        
        # Academic performance
        if optional_data.get("academic_performance"):
            achievement_parts.append(f"Academic Record: {optional_data['academic_performance']}")
        
        # Entrance exam results if available
        if optional_data.get("entrance_exam_results"):
            achievement_parts.append(f"Entrance Exam Performance: {optional_data['entrance_exam_results']}")
            ...
            if output_data.get("recommended_streams"):
                ...
                if isinstance(streams, list) and len(streams) > 0:
                    ...
                    if isinstance(top_stream, dict):
                        ...
                        insight_parts.append(f"Recommended academic focus: {stream_name}")
        
        return "\n".join(insight_parts) if insight_parts else "Assessment insights being processed"
    ...
    def _list_assessment_sources(self, previous_outputs: Dict[str, Any]) -> List[str]:
        ...
        sources = []
        for agent_id in ["test_score_interpreter", "academic_stream_advisor", 
                        "career_pathway_explorer", "educational_roadmap_planner"]:
    ...
    def _estimate_college_costs(self, recommended_colleges: List[Dict]) -> Dict[str, float]:
        """Estimate costs from recommended colleges"""
        if not recommended_colleges:
            return {"tuition_range": 200000, "living_costs": 100000, "total_annual": 300000}
        ...
        if costs:
            ...
            return {"tuition_range": 250000, "living_costs": 120000, "total_annual": 370000}
    ...
    def _extract_family_income(self, financial_context: str) -> str:
        ...
        if any(word in context_lower for word in ["low", "limited", "tight", "struggling"]):
            return "Lower Income"
        elif any(word in context_lower for word in ["high", "comfortable", "affluent", "well-off"]):
            return "Higher Income"
        else:
            return "Middle Income"
    
    def _create_decision_support_framework(self, result: Dict[str, Any]) -> Dict[str, Any]:
