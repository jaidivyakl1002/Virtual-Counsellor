...
from agentic_layer.school_students.agents.test_score_interpreter_agent import TestScoreInterpreterAgent
from agentic_layer.school_students.agents.academic_stream_advisor_agent import AcademicStreamAdvisorAgent
from agentic_layer.school_students.agents.career_pathway_explorer_agent import CareerPathwayExplorerAgent
from agentic_layer.school_students.agents.educational_roadmap_planner_agent import EducationalRoadmapPlannerAgent
from agentic_layer.school_students.agents.college_and_scholarship_navigator_agent import CollegeScholarshipNavigatorAgent
...
class SchoolStudentFleetManager(BaseFleetManager):
    ...
    def _initialize_fleet(self):
        ...
        self.agent_dependencies = {
            "test_score_interpreter": AgentDependency("test_score_interpreter", depends_on=[]),
            "academic_stream_advisor": AgentDependency(
                "academic_stream_advisor", 
                depends_on=["test_score_interpreter"],
                required_outputs=["aptitude_analysis", "interest_mapping", "personality_insights"]
            ),
            ...
            "educational_roadmap_planner": AgentDependency(
                ...
                required_outputs=["recommended_streams", "recommended_career_pathways"]  # Map the actual output keys
            ),
            "college_scholarship_navigator": AgentDependency(
                ...
                required_outputs=["career_goals", "educational_pathway", "academic_timeline"]
            )
        }
    ...
    def _initialize_agents(self):
        try:
            ...
            self.add_agent(test_interpreter_agent, self.agent_dependencies.get("test_score_interpreter"))
            ...
            self.add_agent(stream_advisor_agent, self.agent_dependencies.get("academic_stream_advisor"))
            ...
            self.add_agent(career_explorer_agent, self.agent_dependencies.get("career_pathway_explorer"))
            self.logger.info(f"Initialized agent: {career_explorer_agent.agent_name}")
            
            # Initialize Educational Roadmap Planner Agent
            roadmap_planner_agent = EducationalRoadmapPlannerAgent(llm_model=self.llm_model)
            self.add_agent(roadmap_planner_agent, self.agent_dependencies.get("educational_roadmap_planner"))
            self.logger.info(f"Initialized agent: {roadmap_planner_agent.agent_name}")
            
            # Initialize College Scholarship Navigator Agent
            college_navigator_agent = CollegeScholarshipNavigatorAgent(llm_model=self.llm_model)
            self.add_agent(college_navigator_agent, self.agent_dependencies.get("college_scholarship_navigator"))
    ...
    def _validate_fleet_input(self, user_data: Dict[str, Any]) -> tuple[bool, List[str]]:
        required_fields = ["dbda_scores", "cii_results"]  # All assessment data mandatory
        optional_fields = ["demographic_info", "current_grade", "academic_performance", 
                          "extracurricular_activities", "family_preferences", "geographical_preferences"]
        ...
        for field in required_fields:
            ...
            elif isinstance(user_data[field], (dict, list)) and len(user_data[field]) == 0:
                missing_required.append(field)
        ...
        available_optional = [field for field in optional_fields 
    ...
    def _create_execution_plan(self, user_data: Dict[str, Any]) -> List[str]:
        ...
        if (user_data.get("dbda_scores") and 
            user_data.get("cii_results")):
    ...
    def _generate_fleet_recommendations(self, agent_results: Dict[str, AgentResult]) -> List[str]:
        """Generate overall recommendations from agent results"""
        recommendations = []
        
        # Add recommendations based on successful agent outputs
        if ("test_score_interpreter" in agent_results and 
            agent_results["test_score_interpreter"].status == ProcessingStatus.COMPLETED):
            recommendations.append("Discuss your assessment results with parents and school counselors")
            recommendations.append("Focus on developing your identified cognitive strengths")
            recommendations.append("Explore activities that align with your interest areas")
        
        if ("academic_stream_advisor" in agent_results and 
            agent_results["academic_stream_advisor"].status == ProcessingStatus.COMPLETED):
            recommendations.append("Carefully consider the recommended academic streams")
            recommendations.append("Research subject requirements and career implications for each stream")
            recommendations.append("Talk to current students and professionals in fields that interest you")
        
        if ("career_pathway_explorer" in agent_results and 
            agent_results["career_pathway_explorer"].status == ProcessingStatus.COMPLETED):
            recommendations.append("Explore the suggested career paths through internships or job shadowing")
            recommendations.append("Build relevant skills and knowledge in your areas of interest")
            recommendations.append("Connect with professionals in your target fields")
        
        if ("educational_roadmap_planner" in agent_results and 
            agent_results["educational_roadmap_planner"].status == ProcessingStatus.COMPLETED):
            recommendations.append("Follow the grade-wise milestone plan consistently")
            recommendations.append("Start preparation for relevant entrance exams early")
            recommendations.append("Balance academics with extracurricular activities")
            recommendations.append("Regularly review and adjust your educational timeline")
        
        if ("college_scholarship_navigator" in agent_results and 
            agent_results["college_scholarship_navigator"].status == ProcessingStatus.COMPLETED):
            recommendations.append("Research and shortlist colleges that match your goals and budget")
            recommendations.append("Apply for relevant scholarships and financial aid programs")
            recommendations.append("Visit college campuses and attend career counseling sessions")
            recommendations.append("Maintain good academic performance for scholarship eligibility")
    ...
    def _assess_readiness_for_career_guidance(self, user_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if user_data.get("current_grade"):
            ...
            if "11" in str(grade) or "12" in str(grade):
                ...
                readiness_score += 0.1
                readiness_factors.append("Junior grade level allows for exploratory guidance")
        
        if user_data.get("academic_performance"):
            readiness_score += 0.1
            readiness_factors.append("Academic performance data available for informed guidance")
        ...
        return {
            ...
            "guidance_approach": self._determine_guidance_approach(readiness_score, user_data)
        }
    
    def _determine_guidance_approach(self, readiness_score: float, user_data: Dict[str, Any]) -> str:
    ...
    def get_fleet_info(self) -> Dict[str, Any]:
        return {
            ...
            "optional_inputs": ["demographic_info", "current_grade", "academic_performance", 
                              "extracurricular_activities", "family_preferences", "geographical_preferences"],
            ...
            "dependencies": {agent_id: dep.depends_on for agent_id, dep in self.agent_dependencies.items()},
            "execution_history_count": len(self.execution_history),
            "age_group_focus": "14-18 years (grades 9-12)",
            "guidance_types": ["stream_selection", "career_exploration", "educational_planning", "college_preparation"]
        }
    ...
    def can_execute(self, user_data: Dict[str, Any]) -> tuple[bool, List[str], Dict[str, Any]]:
        ...
        execution_info = {
            ...
            "execution_plan": self._create_execution_plan(user_data) if is_valid else [],
            "estimated_agents": len(self._create_execution_plan(user_data)) if is_valid else 0,
            "estimated_time_minutes": len(self._create_execution_plan(user_data)) * 2 if is_valid else 0,
            "readiness_assessment": readiness_assessment,
            "guidance_focus": self._determine_primary_guidance_focus(user_data) if is_valid else None
        }
        
        return is_valid, missing_data, execution_info
    
    def _determine_primary_guidance_focus(self, user_data: Dict[str, Any]) -> Dict[str, Any]:
        current_grade = user_data.get("current_grade", "")
        focus_areas = []
        
        if "9" in str(current_grade):
            focus_areas = ["foundation_building", "interest_exploration", "early_stream_awareness"]
        elif "10" in str(current_grade):
            focus_areas = ["stream_selection_preparation", "career_awareness", "skill_development"]
        elif "11" in str(current_grade):
            focus_areas = ["stream_optimization", "career_planning", "entrance_exam_preparation"]
        elif "12" in str(current_grade):
            focus_areas = ["college_selection", "career_finalization", "transition_planning"]
        else:
            focus_areas = ["general_career_exploration", "aptitude_development"]
        
        return {
            "primary_focus_areas": focus_areas,
            "timeline_urgency": "high" if "12" in str(current_grade) else "medium" if "11" in str(current_grade) else "low",
            "decision_pressure": "immediate" if "12" in str(current_grade) else "upcoming" if "11" in str(current_grade) else "future_planning"
        }