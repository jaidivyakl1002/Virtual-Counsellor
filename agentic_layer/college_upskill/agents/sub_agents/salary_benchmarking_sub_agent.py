...
class SalaryBenchmarkOutput(BaseModel):
    ...
    domain_salary_ranges: Dict[str, Dict[str, Any]] = Field(description="Salary ranges by domain and experience level")
    geographic_variations: Dict[str, Dict[str, Any]] = Field(description="Salary variations by location")
    growth_trajectories: Dict[str, List[Dict[str, Any]]] = Field(description="Career progression and salary growth")
    total_compensation_analysis: Dict[str, Any] = Field(description="Beyond base salary compensation")
    market_positioning: Dict[str, str] = Field(description="How domains compare in compensation")
...
class SalaryBenchmarkingSubAgent:
    ...
    def __init__(self, llm_model):
        ...
        self.experience_levels = ["Entry Level (0-2 years)", "Mid Level (2-5 years)", 
                                 "Senior Level (5-10 years)", "Expert Level (10+ years)"]
        
        self.indian_markets = {
            "Tier 1": ["Mumbai", "Delhi", "Bangalore", "Chennai", "Hyderabad", "Pune"],
            "Tier 2": ["Ahmedabad", "Kolkata", "Jaipur", "Chandigarh", "Kochi", "Indore"],
            "Remote": ["Remote India", "Remote Global"]
        }
        
        self.prompt = PromptTemplate(
            input_variables=["specific_domains", "intermediate_domains", "broad_categories", "student_level"],
...
Return only valid JSON without any markdown formatting, comments, or explanations."""
    ...
    def analyze_compensation(self, specific_domains: List[str], intermediate_domains: List[str], 
                           broad_categories: List[str], student_context: str = "Final year college student") -> Dict[str, Any]:
    ...
    def _calculate_salary_metrics(self, salary_result: Dict) -> Dict[str, Any]:
        ...
        for domain, salary_info in domain_salaries.items():
            if isinstance(salary_info, dict) and "Entry Level (0-2 years)" in salary_info:
        ...
        metrics["domain_ranking"] = dict(sorted(entry_rankings.items(), 
