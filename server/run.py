...
from fastapi import (
    ...
    File, UploadFile, Form, Depends, Header, BackgroundTasks
)
...
def update_session_status(session_id: str, status: SessionStatus, data: Optional[Dict] = None, error: Optional[str] = None):
...
async def get_verticals_info():
    ...
    try:
        ...
        return APIResponse(
            success=True,
            data={
                "verticals": verticals_info,
                "total_count": len(verticals_info)
            }
        )
    except Exception as e:
...
async def process_school_student_background(
    ...
    try:
        logger.info(f"Starting background processing for school student session: {session_id}")
        ...
        if result.get("success"):
            ...
            update_session_status(session_id, SessionStatus.FAILED, error=result.get("error", "Analysis failed"))
            
    except Exception as e:
        logger.error(f"Background task error for session {session_id}: {e}", exc_info=True)
...
async def process_college_upskilling_background(
    ...
    try:
        logger.info(f"Starting background processing for college upskilling session: {session_id}")
        update_session_status(session_id, SessionStatus.PROCESSING)
        ...
        if result.get("success"):
            logger.info(f"College upskilling analysis completed for session: {session_id}")
            update_session_status(session_id, SessionStatus.COMPLETED, data=result)
        else:
...
async def process_career_transition_background(
    ...
    try:
        logger.info(f"Starting background processing for career transition session: {session_id}")
        update_session_status(session_id, SessionStatus.PROCESSING)
        ...
        if result.get("success"):
            logger.info(f"Career transition analysis completed for session: {session_id}")
            update_session_status(session_id, SessionStatus.COMPLETED, data=result)
        else:
...
async def analyze_school_student(
    ...
    try:
        ...
        initial_message = request.initial_message or "I need comprehensive academic and career guidance based on my assessment results."
        
        # Add background task
        background_tasks.add_task(
            process_school_student_background,
            user_data,
            initial_message,
            session_id
        )
...
async def analyze_career_transition(
    ...
    try:
        ...
        # Set default message
        initial_message = request.initial_message or "I want to explore career transition options and get a detailed transition plan."
        
        # Add background task
        background_tasks.add_task(
            process_career_transition_background,
            user_data,
            initial_message,
            session_id
        )
    ...
    try:
        ...
        return APIResponse(
            ...
            data={
                ...
                "results": session_info["data"] if session_info["status"] == SessionStatus.COMPLETED else None,
                "error": session_info["error"] if session_info["status"] == SessionStatus.FAILED else None,
...
async def chat_with_counselor(request: ChatMessage):
    ...
    try:
        ...
        if result.get("success"):
            ...
            logger.warning(f"Chat failed for session {request.session_id}: {result.get('error')}")
            return APIResponse(
                success=False,
