...
class CareerOptimizationPlannerAgent(BaseAgent):
    ...
    def _initialize_agent(self):
        ...
        self.optimization_prompt = PromptTemplate(
            input_variables=[
                "profile_summary", "current_strengths", "skill_development_plan", 
                "market_opportunities", "career_preferences", "timeline_constraints",
                "academic_context", "analysis_date", "format_instructions"
            ],
...
{format_instructions}
    ...
    @traceable(name="comprehensive_career_strategy", tags=["career_optimization_strategy", "comprehensive", "llm_chain"])
    def _process_core_logic(self, validated_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        market_intelligence = self._extract_agent_output(validated_input, "market_intelligence")
        skill_development = self._extract_agent_output(validated_input, "skill_development_strategist")
        ...
        output_dict["optimization_metadata"] = {
            ...
            "implementation_complexity": self._assess_implementation_complexity(output_dict),
            "success_probability": self._estimate_success_probability(validated_input, output_dict)
        }
        
        # Add implementation timeline
        output_dict["implementation_timeline"] = self._create_implementation_timeline(output_dict)
        
        self._add_processing_note("Career optimization strategy completed successfully")
        return output_dict
    
    @traceable(name="previous_agent_output_extraction", tags=["career_optimization_strategy", "previous_outputs", "llm_chain"])
    ...
    def _create_profile_summary(self, profile_analysis: Dict[str, Any]) -> str:
        ...
        if "comprehensive_analysis" in profile_analysis:
            ...
            if "profile_positioning" in comp_analysis:
                summary_parts.append(f"Profile Positioning: {comp_analysis['profile_positioning']}")
            ...
            if "primary_domain" in comp_analysis:
                summary_parts.append(f"Primary Domain: {comp_analysis['primary_domain']}")
    ...
    def _extract_current_strengths(self, profile_analysis: Dict[str, Any]) -> str:
        ...
        return ", ".join(strengths) if strengths else "Foundational skills and strong learning ability"

    def _extract_skill_development_summary(self, skill_development: Dict[str, Any]) -> str:
        ...
        if "development_roadmap" in skill_development and skill_development["development_roadmap"]:
            ...
            if isinstance(roadmap, dict):
                if "immediate_actions" in roadmap and roadmap["immediate_actions"]:
                    ...
                    if isinstance(immediate, list):
                        summary_parts.append(f"Immediate Actions: {'; '.join(immediate[:3])}")
        
        return "; ".join(summary_parts) if summary_parts else "Structured skill development plan in progress"
    ...
    def _extract_market_opportunities(self, market_intelligence: Dict[str, Any]) -> str:
        ...
        return ", ".join(opportunities) if opportunities else "Diverse opportunities in technology and business sectors"
    ...
    def _extract_career_preferences(self, validated_input: Dict[str, Any]) -> str:
        ...
        if "company_preferences" in optional_data:
            preferences.append(f"Company preference: {optional_data['company_preferences']}")
        
        if "work_life_balance_priorities" in optional_data:
            preferences.append(f"Work-life balance: {optional_data['work_life_balance_priorities']}")
        
        return "; ".join(preferences) if preferences else "Open to diverse career opportunities with growth potential"
    ...
    def _extract_timeline_constraints(self, validated_input: Dict[str, Any]) -> str:
        ...
        if "academic_status" in optional_data:
            ...
            if current_year == 4:
                return "Final year - 6-12 months for job search preparation and placement"
            elif current_year == 3:
                return "Pre-final year - 12-18 months for internships and skill building"
    ...
    def _extract_academic_context(self, validated_input: Dict[str, Any]) -> str:
        ...
        if "academic_status" in optional_data:
            ...
            return "; ".join(context_parts) if context_parts else "College student in good academic standing"
    ...
    def _assess_personalization_level(self, validated_input: Dict[str, Any]) -> str:
        ...
        if validated_input.get("previous_outputs", {}).get("market_intelligence"):
            data_points += 2
        if validated_input.get("previous_outputs", {}).get("skill_development_strategist"):
            data_points += 2
    ...
    def _assess_implementation_complexity(self, output_dict: Dict[str, Any]) -> str:
        ...
        if "monthly_action_plan" in output_dict:
            ...
            if isinstance(monthly_plan, dict):
                complexity_factors += sum(len(actions) for actions in monthly_plan.values() if isinstance(actions, list))
    ...
    def _estimate_success_probability(self, validated_input: Dict[str, Any], output_dict: Dict[str, Any]) -> float:
    ...
    def _create_implementation_timeline(self, output_dict: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    def _calculate_confidence_score(self, validated_data: Dict[str, Any], output_data: Dict[str, Any]) -> float:
        ...
        if validated_data.get('previous_outputs', {}).get('skill_development_strategist'):
            base_confidence += 0.05
        
        # Check strategy completeness
        expected_components = ["career_goals", "career_strategy", "networking_plan", "job_search_strategy"]
