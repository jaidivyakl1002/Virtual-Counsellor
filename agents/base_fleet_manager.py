...
class BaseFleetManager(ABC):
    ...
    def _setup_fleet_tracing(self):
        ...
        try:
            ...
            self.logger.info(f"LangSmith tracing initialized for fleet: {self.fleet_id}")
        except Exception as e:
            self.logger.warning(f"LangSmith setup failed for fleet {self.fleet_id}: {e}")
    ...
    def execute_workflow(self, user_data: Dict[str, Any], 
                        conversation_context: Dict[str, Any] = None) -> FleetResult:
        ...
        try:
            ...
            if not is_valid:
                result = self._create_failed_result(f"Missing required data: {', '.join(missing_data)}")
                self._log_fleet_to_langsmith(fleet_run_id, result, error=result.metadata.get("failure_reason"))
                return result
    ...
    def _execute_agents_with_tracing(self, execution_plan: List[str], user_data: Dict[str, Any], 
                                   conversation_context: Dict[str, Any], parent_run_id: str) -> Dict[str, AgentResult]:
        ...
        for i, agent_id in enumerate(execution_plan):
            ...
            self.logger.info(f"Executing agent: {agent.agent_name} (run: {agent_run_id})")
            ...
            if self.langsmith_client and agent_run_id:
                try:
                    agent_outputs = {
                        ...
                        "output_size": len(result.output_data) if result.output_data else 0
                    }
            ...
            self.logger.info(f"Agent {agent_id} completed with status: {result.status.value}")
        
        return agent_results
    
    def _log_fleet_to_langsmith(self, run_id: str, result: FleetResult, error: str = None):
        ...
        try:
            outputs = {
                ...
                "successful_agents": len([r for r in result.agent_results.values() 
                                        if r.status == ProcessingStatus.COMPLETED]),
                "failed_agents": len([r for r in result.agent_results.values() 
    ...
    def _calculate_fleet_confidence(self, agent_results: Dict[str, AgentResult]) -> float:
        """Calculate overall fleet confidence score"""
        if not agent_results:
            return 0.0
        
        completed_results = [r for r in agent_results.values() 
                           if r.status == ProcessingStatus.COMPLETED]
        
        if not completed_results:
            return 0.0
        
        avg_confidence = sum(r.confidence_score for r in completed_results) / len(completed_results)
        completion_rate = len(completed_results) / len(agent_results)
        
        return round(avg_confidence * completion_rate, 2)
    
    def _determine_fleet_status(self, agent_results: Dict[str, AgentResult]) -> FleetStatus:
        """Determine overall fleet execution status"""
        if not agent_results:
            return FleetStatus.FAILED
        
        completed = sum(1 for r in agent_results.values() if r.status == ProcessingStatus.COMPLETED)
    ...
    def _create_execution_summary(self, agent_results: Dict[str, AgentResult]) -> Dict[str, Any]:
        """Create execution summary"""
        return {
            "total_agents": len(agent_results),
            "successful_agents": sum(1 for r in agent_results.values() 
                                   if r.status == ProcessingStatus.COMPLETED),
            "failed_agents": sum(1 for r in agent_results.values() 
                               if r.status == ProcessingStatus.FAILED),
            "avg_processing_time": sum(r.processing_time for r in agent_results.values()) / len(agent_results),
            "total_warnings": sum(len(r.warnings) for r in agent_results.values()),
        }
    
    def _generate_next_actions(self, agent_results: Dict[str, AgentResult]) -> List[str]:
    ...
    def _create_failed_result(self, error_message: str) -> FleetResult:
        ...
        return FleetResult(
            ...
            metadata={"failure_reason": error_message, "timestamp": datetime.now().isoformat()}
        )
