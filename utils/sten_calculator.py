class StenCalculator:
    ...
    def get_sten_score(self, raw_score, ability, grade, gender='male'):
        ...
        if grade not in self.norms:
            raise ValueError(f"Grade {grade} not supported. Available grades: {list(self.norms.keys())}")
        
        if gender not in self.norms[grade]:
            raise ValueError(f"Gender '{gender}' not supported. Available: {list(self.norms[grade].keys())}")
        
        if ability not in self.norms[grade][gender]:
            raise ValueError(f"Ability '{ability}' not supported. Available: {list(self.norms[grade][gender].keys())}")
...
if __name__ == "__main__":
    ...
    results = calculator.calculate_student_stens(example_scores, grade=11, gender='female')
    ...
    print("\nSten scores only:")
    sten_only = [str(data['sten_score']) if data['sten_score'] is not None else 'N/A' 
